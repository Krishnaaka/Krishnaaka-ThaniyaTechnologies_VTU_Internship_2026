...
import { Ticker } from '../components/ScrollEffects'
...
const TICKER_ITEMS = [
  'React', 'FastAPI', 'AWS ECS', 'Docker', 'Python', 'Node.js',
  'OpenCV', 'YOLOv8', 'MongoDB', 'LangChain', 'Ollama', 'Kubernetes',
  'Terraform', 'JWT', 'REST APIs', 'GenAI',
]
...
export default function Hero() {
  ...
  const imgY    = useTransform(scrollYProgress, [0, 1], ['0px', '-65px'])
  const opacity = useTransform(scrollYProgress, [0, 0.65], [1, 0])
  const bgYS    = useSpring(bgY,  { stiffness: 55, damping: 18 })
  const imgYS   = useSpring(imgY, { stiffness: 55, damping: 18 })
  ...
  return (
    <section ref={sectionRef} id="hero"
      style={{position:'relative',minHeight:'100vh',display:'flex',alignItems:'center',overflow:'hidden',paddingTop:88}}>

      {/* ── Parallax background layer ── */}
      <motion.div style={{ position:'absolute', inset:0, zIndex:0, y: bgYS }}>
        <Aurora/>
        <Stars/>
      </motion.div>
      ...
      {/* ── Main content — fades out as user scrolls ── */}
      <motion.div style={{ position:'relative', zIndex:1, opacity, width:'100%' }}>
        <div className="container">
          <div style={{display:'grid',gridTemplateColumns:'1.1fr 0.9fr',gap:'5rem',alignItems:'center'}}>

            {/* LEFT — parallax text slides up on scroll */}
              ...
              <motion.div {...fadeUp(.38)} style={{display:'flex',gap:'.55rem',alignItems:'center'}}>
                ...
                {[
                  ...
                  <motion.a key={i} href={s.href} target="_blank" title={s.label}
                    whileHover={{y:-4,scale:1.12}} whileTap={{scale:.93}}
            ...
            {/* RIGHT — profile card slides opposite direction on scroll */}
            <motion.div style={{ y: imgYS }}>
              <motion.div
      ...
      {/* ── Tech ticker strip — above scroll cue ── */}
      <motion.div
        initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:1.4 }}
        style={{ position:'absolute', bottom:'5.5rem', left:0, right:0, zIndex:2 }}>
        <Ticker items={TICKER_ITEMS} speed={40} color="var(--primary)" />
      </motion.div>

      {/* ── Scroll indicator ── */}
      <motion.a href="#about" animate={{y:[0,10,0]}} transition={{repeat:Infinity,duration:1.8}}
        style={{position:'absolute',bottom:'2rem',left:'50%',transform:'translateX(-50%)',display:'flex',flexDirection:'column',alignItems:'center',gap:'.4rem',color:'var(--muted)',fontSize:'.67rem',letterSpacing:'.15em',textTransform:'uppercase',zIndex:3,cursor:'none'}}>
      ...
      <style>{`
        ...
        @media(max-width:960px){
          #hero .container>div{grid-template-columns:1fr!important;gap:2rem!important}
